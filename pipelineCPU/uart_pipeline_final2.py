...
class UARTGui(tk.Tk):
    def __init__(self):
        ...
        self.bin_label = tk.Label(self, font=("Consolas", 12))
        self.dec_label = tk.Label(self, font=("Consolas", 12))
        self.hex_label = tk.Label(self, font=("Consolas", 12))
            ...
            if self.ser.in_waiting:
                ...
                while len(buf) >= 156:
                    ...
                    for i, name in enumerate(dbg_signals, start=33):
                        ...
                        if asm == "ecall" and name == "EXE" :
                            a10, = struct.unpack_from('<I', block, 44)
                            a17, = struct.unpack_from('<I', block, 72)
                            a10 = byte_swap_32(a10)
                            a17 = byte_swap_32(a17)

                            self.bin_label.config(
                                text=f"BIN: {a10 & 0xFF:08b}",
                                fg='blue' if a17 == 35 else 'black'
                            )
                            self.dec_label.config(
                                text=f"DEC: {to_signed32(a10)}",
                                fg='blue' if a17 == 1 else 'black'
                            )
                            self.hex_label.config(
                                text=f"HEX: {a10:08X}",
                                fg='blue' if a17 == 34 else 'black'
                            )
