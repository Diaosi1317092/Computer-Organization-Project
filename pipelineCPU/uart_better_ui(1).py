...
class UARTGui(tk.Tk):
    def __init__(self):
        ...
        debug_frame.grid(row=3, column=1, padx=(5,10), pady=10, sticky='nsew')
        debug_frame.columnconfigure(0, weight=1)

        dbg_signals = ["PC ", "WB ", "MEM", "EXE", "ID ", "IF "]
        self.dbg_labels = {}
        for row, name in enumerate(dbg_signals):
            bg = '#f0f0ff' if row%2==0 else '#ffffff'
            ldl = tk.Label(debug_frame, text=f"{name}: 0x00000000",
                           font=('Consolas',11), bg=bg, anchor='w')
            ldl.grid(row=row, column=0, sticky='ew', padx=5, pady=1)
            self.dbg_labels[name] = ldl
            ...
            if self.ser.in_waiting:
                ...
                while len(buf) >= 156:
                    ...
                    for i in range(32, 38):
                        ...
                        v3 = (val & 0x000000FF)<<24
                        val = v0|v1|v2|v3
                        ldl = self.dbg_labels[dbg_signals[i - 32]]
                        # alternate highlight for changed value
                        ldl.config(text=f"{dbg_signals[i - 32]}: 0x{val:08X}",
                                   fg='blue' if val != int(ldl.cget('text')[-8:],16) else 'black')
            time.sleep(0.005)
