                ...
                while len(buf) >= 156:
                    ...
                    for i, name in enumerate(dbg_signals, start=33):
                        ...
                        if asm == "ecall" and name == "EXE" :
                            ...
                            self.bin_label.config(text=f"BIN: {a10&0xFF:08b}", fg='blue' if a17 == 35 else "black")
                            self.dec_label.config(text=f"DEC: {to_signed32(a10)}", fg='blue' if a17 == 1 else "black")
                            self.hex_label.config(text=f"HEX: {a10:08X}", fg='blue' if a17 == 34 else "black")
